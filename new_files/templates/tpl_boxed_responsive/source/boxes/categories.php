...
if (!$box_smarty->is_cached(CURRENT_TEMPLATE.'/boxes/box_categories.html', $cache_id) || !$cache) {
  ...
  if (xtc_db_num_rows($categories_query, true) > 0) {
    while ($categories = xtc_db_fetch_array($categories_query, true)) {
      if (defined('MODULE_CATEGORIES_FLAG_STATUS') && MODULE_CATEGORIES_FLAG_STATUS == 'true') {
        $flagArray = (($categories['flags'] > 0) ? '<span class="categoryFlag'.$categories['flags'].'">'.constant('CATEGORIES_FLAG_'.$categories['flags']).'</span>' : '');
      } else {
        $flagArray = '';
      }
      ...
      foreach ($id as $key => $value) {
        ...
        if (xtc_db_num_rows($categories_query, true) > 0) {
          ...
          while ($row = xtc_db_fetch_array($categories_query, true)) {
            if (defined('MODULE_CATEGORIES_FLAG_STATUS') && MODULE_CATEGORIES_FLAG_STATUS == 'true') {
              $flagArray = (($row['flags'] > 0) ? '<span class="categoryFlag'.$row['flags'].'">'.constant('CATEGORIES_FLAG_'.$row['flags']).'</span>' : '');
            } else {
              $flagArray = '';
            }
